...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.inplanes = num_filters[0] # 32
        self.conv1 = nn.Conv2d(3, num_filters[0], kernel_size=3, padding=1, # 3*32*32 --> 32*32*32
                               bias=False)
        self.bn1 = nn.BatchNorm2d(num_filters[0])
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, num_filters[1], n) # basicblock, 64, 5
        self.layer2 = self._make_layer(block, num_filters[2], n, stride=2) # 128, 5, 2
        self.layer3 = self._make_layer(block, num_filters[3], n, stride=2) # 256, 5, 2
        self.avgpool = nn.AvgPool2d(8) # 8*8
        self.fc = nn.Linear(num_filters[3] * block.expansion, num_classes) # 256*1 expansion=1
...
def save_checkpoint(state, path, epoch):
    # Save checkpoint.
    print('Saving..')
    torch.save(state, path+'/best-ckpt')
...
net_cm = resnet32x4(num_classes = 10)
