...
"""## <font color = 'lightblue'>**ResNet32x4 Model**</font>"""
...
model = resnet32x4(num_classes = 10)
...
checkpoint = os.path.join('./weights/resnet_32x4/all_in_one')
...
for epoch in range(epochs):
  ...
  for iter, data in progress:
    ...
    for n in range(n_holes):   # n_holes = 1, length = 8
    ...
    # cutout already applied
    # now applying cutmix using different set of images and lambda values as of mixup
    r = np.random.rand(1)
    if (beta > 0 and r < cm_prob):
    ...
    # done with the augmentation of images

    optimiser_simple.zero_grad()
    outputs = model(inputs)
    outputs = outputs.to(device)
    loss1 = criterion_cls(outputs, targets)
    loss2 = mixup_criterion(criterion_cls, outputs, target_a_mu, target_b_mu, lam_mu)
    loss3 = mixup_criterion(criterion_cls, outputs, target_a_cm, target_b_cm, lam_cm)
