...
def validate_curropted(distortions, test_d, test_loader, net, s):
    print("Validating {} rn_32x4 model...".format(s))
    ...
    for dist in distortions:
        ...
        test_d.data = np.load(f'./CIFAR-100-C/{dist}.npy')
        test_d.targets =  torch.LongTensor(np.load('./CIFAR-100-C/labels.npy'))
...
for i in ['without', 'mixup', 'cutout', 'cutmix', 'mu_cm', 'mu_co', 'cm_co', 'all_in_one', 'sequentially']:
    test_d = CIFAR100(root='../cifar100/data/', train=False, transform=transform_test, download=True)
    ...
    x = torch.load('../cifar100/weights/resnet_32x4/{}/best_ckpt'.format(i))
